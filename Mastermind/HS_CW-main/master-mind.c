...
// {
//   if (value == HIGH) {
//         *gpio |= (1 << led);  // Set LED pin to HIGH
//     } else {
//         *gpio &= ~(1 << led); // Set LED pin to LOW
//     }
// };
...
//   if ((pin & 0xFFFFFFC0) == 0) {
//         if ((*(gpio + 13) & (1 << pin)) == 0) {
//             return LOW;  // Button not pressed
//         } else {
//             return HIGH; // Button pressed
//         }
//     } else {
...
//             "mov r2, #1\n\t"
//             "lsl r2, %[pin]\n\t"
//             "str r2, [%[gpio], #40]" // GPIO_CLR0 is typically 0x28 or 40 bytes from GPIO base
//             :
...
//             "mov r2, #1\n\t"
//             "lsl r2, %[pin]\n\t"
//             "str r2, [%[gpio], #28]" // GPIO_SET0 is typically 0x1C or 28 bytes from GPIO base
//             :
//             : [gpio] "r" (gpio), [pin] "r" (pin)
//             : "r2", "memory"
//         );
...
//             "mov r2, #1\n\t"
//             "lsl r2, %[led]\n\t"
//             "str r2, [%[gpio], #28]" // Assuming GPIO_SET0
//             :
...
//             "mov r2, #1\n\t"
//             "lsl r2, %[led]\n\t"
//             "str r2, [%[gpio], #40]" // Assuming GPIO_CLR0
//             :
//             : [gpio] "r" (gpio), [led] "r" (led)
//             : "r2", "memory"
//         );
...
// {    int value;
//     asm volatile (
//         "ldr r2, [%[gpio], #52]\n\t" // Load the value from GPLEV0 register (0x34 or 52 bytes from base)
//         "mov r3, #1\n\t"
//         "lsl r3, %[pin]\n\t" // Shift left to the pin position
//         "and %[value], r2, r3\n\t" // And operation to check if the pin is high
//         "cmp %[value], #0\n\t"
//         "moveq %[value], #0\n\t" // If result is 0, set value to LOW
//         "movne %[value], #1\n\t" // Else, set value to HIGH
//         : [value] "=r" (value) // Output
//         : [gpio] "r" (gpio), [pin] "r" (pin) // Input
//         : "r2", "r3", "cc" // Clobbered registers
//     );
//     return value;
// }
...
void initSeq() {
  ...
  if (theSeq == NULL) //to check if memory has beeen allocated
    theSeq = (int *)malloc(seqlen * sizeof(int));
    
  for (int i = 0; i < seqlen; i++) { // Generate random digits for the sequence
    theSeq[i] = rand() % colors+1; // Generate a random number between 0 and colors-1
  }
}
...
void showMatches(int /* or int* */ code, /* only for debugging */ int *seq1, int *seq2, /* optional, to control layout */ int lcd_format) {
    ...
    int approximate =  code % 10; // Bitwise AND with 0xF (which is 15 in decimal or 1111 in binary) to get the 'approximate' value
...
uint64_t timeInMicroseconds(){
  struct timeval currentTime; //variable created to old the current time in both seconds and microseconds
  gettimeofday(&currentTime, NULL); //the functions  get current time and the 2nd paramter is for the timezone(which is NULL in this case)
  
  return (uint64_t)currentTime.tv_sec * 1000000 + (uint64_t)currentTime.tv_usec;//updates  structure with the current time, this line calculates and returns the time in microseconds
}
...
void initITimer(uint64_t timeout){
  // Set up signal handler for SIGALRM
    ...
    timer.it_value.tv_sec = timeout / 1000000; // Convert microseconds to seconds
    timer.it_value.tv_usec = timeout % 1000000; // Remaining microseconds
    timer.it_interval.tv_sec = timeout / 1000000; // Repeat interval in seconds
    timer.it_interval.tv_usec = timeout % 1000000; // Remaining microseconds
...
{ // this is just a suggestion of some variable that you may want to use
  ...
  int G_LED = LED, R_LED = LED2, pinButton = BUTTON;
  ...
  /* ***  COMPLETE the code here  ***  */
  pinMode(gpio, G_LED, OUTPUT);
  pinMode(gpio, R_LED, OUTPUT);
...
//	mode which sends the commands as 2 x 4-bit values.
  ...
  /* ******************************************************* */
  // Generate a guess sequence (for testing purpose)

  //starting of game both leds should be off
  digitalWrite(gpio, G_LED, LOW);
  digitalWrite(gpio, R_LED, LOW);
  ...
  while (!found && attempts < 5)
  {
    ...
    sprintf(buf, "Attempt: %d", attempts);
    lcdPosition(lcd, 0, 0);
    lcdPuts(lcd, buf);
    delay(1500);
    lcdClear(lcd);
    ...
    while (1)
    {
      ...
      while (time(NULL) < endTime)
      {
        ...
        if (buttonCount >= seqlen)
        {
          break;
        }
      }
      printf("Button pressed %d count\n", buttonCount);

      // Blink red LED indicating the end of the time window
      digitalWrite(gpio, R_LED, HIGH);
      delay(2000);
      digitalWrite(gpio, R_LED, LOW);

      // Blink the number of times the button was pressed on green
      blinkN(gpio, G_LED, buttonCount);
      delay(2000);

      // Store the number of button presses in attSeq
      attSeq[num - 1] = buttonCount;
      // Repeat for the length of sequence
      if (num <= seqlen);
      {
      ...
      if (num == seqlen)
      {
        ...
        while (i < seqlen) { // Iterate through attSeq up to seqlen
        printf("%d\n", attSeq[i]); // Print each element of attSeq
        i++; // Increment loop counter
    }

        blinkN(gpio, R_LED, 2); // Blink LED
        delay(1500);
        break; // Exit the loop
  ...
  if (found) {
    ...
    printf("Max Attempts over\n");
    sprintf(buf,"Attempts over\n");
    lcdPosition(lcd, 0, 1);
    lcdPuts(lcd, buf);
    delay(2000);
  }
